    ...
    try:
        ...
        if exec_thread.is_alive():
            ...
            return {
                ...
                "stdout": stdout_capture.getvalue()[:max_stdout_size] if stdout_capture.getvalue() else "",
                "stderr": stderr_capture.getvalue()[:max_stderr_size] if stderr_capture.getvalue() else "",
                "timeout": timeout,
        ...
        if dangerous_patterns and allow_dangerous:
            ...
            result["safety_warning"] = "Code with dangerous patterns was executed with allow_dangerous=True"
...
def connect_nodes(
    ...
    try:
        ...
        if src_node is None:
            return {
                "status": "error",
                "message": f"Source node not found: {src_path}"
            }
        ...
        if dst_node is None:
            return {
                "status": "error",
                "message": f"Destination node not found: {dst_path}"
            }
        ...
        if dst_input_index < len(existing_inputs) and existing_inputs[dst_input_index] is not None:
            ...
            logger.info(f"Disconnecting existing connection from {existing_src.path()} to {dst_path}[{dst_input_index}]")
...
def disconnect_node_input(
    ...
    try:
        ...
        if was_connected:
            result["message"] = f"Disconnected input {input_index} on {node_path} (was connected to {existing_input.path()})"
...
def set_node_flags(
    ...
    try:
        ...
        if flags_unavailable:
            ...
            result["warning"] = f"Some flags not available on this node type: {', '.join(flags_unavailable)}"
        
        if not flags_set:
            result["message"] = "No flags were set (all values were None or unavailable)"
        else:
            result["message"] = f"Set flags on {node_path}: {', '.join(f'{k}={v}' for k, v in flags_set.items())}"
    ...
    try:
            ...
            try:
                ...
                for attrib in geo.pointAttribs():
                    try:
                        ...
                        data_type_name = data_type.name() if hasattr(data_type, 'name') else str(data_type)
        ...
        if max_sample_points > 0 and point_count > 0:
            ...
            if point_count > 1000000:
                result["warning"] = f"Geometry has {point_count} points (>1M). Sampling limited to {max_sample_points} points."
                ...
                for i, pt in enumerate(points_list):
                    ...
                    try:
                        ...
                        try:
                            ...
                            if pos is not None:
                                point_data["P"] = list(pos) if isinstance(pos, (tuple, list)) else pos
                        except Exception:
                            pass
