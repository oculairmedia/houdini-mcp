...
def main():
    ...
    geo_result = call_tool("create_node", node_type="geo", parent_path="/obj", name="param_example")
    ...
    rad_schema = call_tool("get_parameter_schema", node_path=sphere_path, parm_name="rad")
    ...
    call_tool("set_parameter", node_path=sphere_path, param_name="rad", value=new_radius)
    print(f"  ✓ Set rad = {new_radius}")

    # Set sphere type (menu parameter)
    print("\n  Discovering 'type' parameter menu items...")
    type_schema = call_tool("get_parameter_schema", node_path=sphere_path, parm_name="type")
    ...
    if type_schema["parameters"]:
        ...
        if type_param.get("menu_items"):
            ...
            call_tool("set_parameter", node_path=sphere_path, param_name="type", value=new_type)
            print(f"  ✓ Set type = {new_type}")

    # Set translation (vector parameter)
    print("\n  Setting translation to [5.0, 0.0, 0.0]...")
    new_translate = [5.0, 0.0, 0.0]
    call_tool("set_parameter", node_path=sphere_path, param_name="t", value=new_translate)
    print(f"  ✓ Set t = {new_translate}")

    # Set uniform scale (float parameter)
    print("\n  Setting uniform scale to 2.5...")
    call_tool("set_parameter", node_path=sphere_path, param_name="scale", value=2.5)
    print("  ✓ Set scale = 2.5")
    ...
    node_info = call_tool(
        "get_node_info", node_path=sphere_path, include_params=True, max_params=50
    )
    ...
    geo_summary = call_tool(
        "get_geo_summary", node_path=sphere_path, max_sample_points=0, include_attributes=False
    )
    ...
    if geo_summary.get("bounding_box"):
        ...
        print(
            f"    {'✓' if size_match else '✗'} Size matches radius×scale (expected ≈ {expected_diameter})"
        )
    ...
    print("  1. ✓ Discovered all parameters with get_parameter_schema")
    print("  2. ✓ Queried specific parameter schemas")
    print("  3. ✓ Set parameters intelligently (vector, menu, float)")
    print("  4. ✓ Verified parameters with get_node_info")
    print("  5. ✓ Verified geometry reflects changes")
