...
def call_tool(tool_name: str, **kwargs) -> dict[str, Any]:
    ...
    response = requests.post(f"{MCP_URL}/tools/{tool_name}", json=kwargs)
...
def check_for_errors(node_path: str) -> dict[str, Any] | None:
...
def print_cook_info(cook_info: dict[str, Any]) -> None:
    ...
    state_symbol = {"cooked": "✓", "error": "✗", "dirty": "⚠", "uncooked": "○"}.get(state, "?")
...
def main():
    ...
    geo_result = call_tool("create_node", node_type="geo", parent_path="/obj", name="error_example")
    ...
    noise_result = call_tool("create_node", node_type="noise", parent_path=geo_path, name="noise1")
    ...
    if cook_info:
        ...
        geo_result = call_tool("get_geo_summary", node_path=noise_path, max_sample_points=0)

        if geo_result.get("status") == "success" and geo_result["point_count"] == 0:
            print("\n  ⚠ Warning: Noise has no geometry (0 points)")
            print("     This is expected - noise needs input geometry!")

    print("\n[Step 3] Fixing: Adding grid input...")
    grid_result = call_tool("create_node", node_type="grid", parent_path=geo_path, name="grid1")
    grid_path = grid_result["node_path"]

    call_tool("connect_nodes", src_path=grid_path, dst_path=noise_path, dst_input_index=0)
    print("✓ Connected: grid → noise")
    ...
    sphere_result = call_tool(
        "create_node", node_type="sphere", parent_path=geo_path, name="sphere1"
    )
    ...
    if bad_result.get("status") == "error":
        ...
        print("     This error helps us understand the correct parameter format!")

    print("\n[Step 3] Checking parameter schema to find correct type...")
    schema = call_tool("get_parameter_schema", node_path=sphere_path, parm_name="rad")

    if schema.get("status") == "success" and schema["parameters"]:
        param = schema["parameters"][0]
        print("  ℹ Parameter 'rad' schema:")
    ...
    cam_result = call_tool("create_node", node_type="cam", parent_path="/obj", name="cam1")
    ...
    if bad_connection.get("status") == "error":
        ...
        print("     The error message tells us we can't mix SOP and Object nodes!")

    print("\n[Step 3] Fix: Only connect nodes of the same category")
    print("     → SOP nodes can only connect to other SOP nodes")
    print("     → OBJ nodes can only connect to other OBJ nodes")
    print("     This validation prevents invalid network structures!")
    ...
    test_result = call_tool("create_node", node_type="box", parent_path=geo_path, name="box1")
    ...
    if cook_info:
        ...
        if cook_info["cook_state"] == "cooked":
            ...
            geo = call_tool("get_geo_summary", node_path=test_path, max_sample_points=3)
            ...
            print("     Would need to handle errors before accessing geometry")
    ...
    print("  1. ✓ Use get_node_info(include_errors=True) to detect issues")
    print("  2. ✓ Examine cook_info to understand error type")
    print("  3. ✓ Use get_parameter_schema to validate parameter types")
    print("  4. ✓ Check cook_state before accessing geometry")
    print("  5. ✓ Handle connection validation errors gracefully")

    print("\nBest practices:")
    print("  • Always check return status from MCP tools")
    print("  • Use include_errors=True when debugging")
    print("  • Validate parameter types with schema before setting")
    print("  • Test node connections for category compatibility")
    print("  • Check cook state before reading geometry")
