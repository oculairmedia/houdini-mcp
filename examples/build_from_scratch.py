...
def main():
    ...
    geo_result = call_tool("create_node", node_type="geo", parent_path="/obj", name="example_geo")
    ...
    print("✓ Set sphere radius: [2.0, 2.0, 2.0]")

    # Step 4: Create transform node
    print("\n[Step 4] Creating transform (xform) node...")
    xform_result = call_tool("create_node", node_type="xform", parent_path=geo_path, name="xform1")
    ...
    print("✓ Set translate: [0.0, 3.0, 0.0]")

    # Step 6: Create color node
    print("\n[Step 6] Creating color node...")
    color_result = call_tool("create_node", node_type="color", parent_path=geo_path, name="color1")
    ...
    print("✓ Set color: [1.0, 0.0, 0.0]")

    # Step 8: Create OUT null node
    print("\n[Step 8] Creating OUT null node...")
    out_result = call_tool("create_node", node_type="null", parent_path=geo_path, name="OUT")
    ...
    call_tool("connect_nodes", src_path=sphere_path, dst_path=xform_path, dst_input_index=0)
    print("  ✓ sphere → xform")

    print("  Connecting xform → color...")
    call_tool("connect_nodes", src_path=xform_path, dst_path=color_path, dst_input_index=0)
    print("  ✓ xform → color")

    print("  Connecting color → OUT...")
    call_tool("connect_nodes", src_path=color_path, dst_path=out_path, dst_input_index=0)
    print("  ✓ color → OUT")

    # Step 10: Set display flag on OUT
    print("\n[Step 10] Setting display flag on OUT node...")
    call_tool("set_node_flags", node_path=out_path, display=True, render=True)
    ...
    if geo_summary.get("sample_points"):
        ...
        for pt in geo_summary["sample_points"][:3]:
