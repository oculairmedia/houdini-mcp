...
def main():
    ...
    geo_result = call_tool(
        "create_node", node_type="geo", parent_path="/obj", name="augment_example"
    )
    ...
    children_result = call_tool("list_children", node_path=geo_path, recursive=False)
    ...
    for child in children_result["children"]:
        ...
        if child["inputs"]:
            for inp in child["inputs"]:
                print(
                    f"      Input {inp['index']}: {inp['source_node']} [output {inp['output_index']}]"
                )
    ...
    noise_info = call_tool(
        "get_node_info", node_path=noise_path, include_params=False, include_input_details=True
    )
    ...
    if noise_info.get("input_connections"):
        ...
        for conn in noise_info["input_connections"]:
            print(
                f"    Input {conn['input_index']}: {conn['source_node']} [output {conn['source_output_index']}]"
            )
    ...
    geo_before = call_tool(
        "get_geo_summary", node_path=noise_path, max_sample_points=5, include_attributes=True
    )
    ...
    mountain_result = call_tool(
        "create_node", node_type="mountain", parent_path=geo_path, name="mountain1"
    )
    ...
    call_tool("set_parameter", node_path=mountain_path, param_name="height", value=2.0)
    print("✓ Set mountain height: 2.0")
    ...
    call_tool("disconnect_node_input", node_path=noise_path, input_index=0)
    print("  ✓ Disconnected")

    # Connect grid → mountain
    print("  Connecting grid → mountain...")
    call_tool("connect_nodes", src_path=grid_path, dst_path=mountain_path, dst_input_index=0)
    print("  ✓ grid → mountain")

    # Connect mountain → noise
    print("  Connecting mountain → noise...")
    call_tool("connect_nodes", src_path=mountain_path, dst_path=noise_path, dst_input_index=0)
    ...
    final_children = call_tool("list_children", node_path=geo_path, recursive=False)
    ...
    print("New flow: grid → mountain → noise")
